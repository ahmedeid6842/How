...
export class AnswerService {
  ...
  async createAnswer(questionId: string, body: CreateAnswernDto, user: User) {
    const [questionExist] = await this.questionService.getQuestion({
      questionId,
    });
    if (!questionExist) {
      throw new NotFoundError(
        ANSWER_ERRORS.PREFIX.BUSINESS,
        ANSWER_ERRORS.NUMBER.QUESTION_NOT_FOUND,
        'answer.question_not_found',
      );
    }
  ...
  async likeAnswer(answerId: string, user: User) {
    ...
    if (!answer) {
      throw new NotFoundError(
        ANSWER_ERRORS.PREFIX.BUSINESS,
        ANSWER_ERRORS.NUMBER.ANSWER_NOT_FOUND,
    ...
    if (likeExists) {
      throw new BadRequestError(
        ANSWER_ERRORS.PREFIX.BUSINESS,
        ANSWER_ERRORS.NUMBER.ALREADY_LIKED,
        'answer.already_liked',
      );
    }
