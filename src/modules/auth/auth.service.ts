...
export class AuthService {
  ...
  async register(email: string, userName: string, password: string) {
    ...
    if (userByEmail.length) {
      throw new BadRequestError(
        ...
        AUTH_ERRORS.NUMBER.EMAIL_ALREADY_EXISTS,
    ...
    if (userByName.length) {
      throw new BadRequestError(
        AUTH_ERRORS.PREFIX.BUSINESS,
        AUTH_ERRORS.NUMBER.USERNAME_ALREADY_EXISTS,
  ...
  async login(userCredentials: LoginUserDto) {
    ...
    if (!user.length) {
      throw new NotFoundError(
        AUTH_ERRORS.PREFIX.BUSINESS,
        AUTH_ERRORS.NUMBER.USER_NOT_FOUND_LOGIN,
        'auth.user_not_found',
      );
    }
    ...
    if (!verifiedUser) {
      throw new BadRequestError(
        AUTH_ERRORS.PREFIX.BUSINESS,
        AUTH_ERRORS.NUMBER.INCORRECT_PASSWORD,
  ...
  async sendResetPasswordEmail(userData: any) {
    ...
    if (!user) {
      throw new NotFoundError(
        AUTH_ERRORS.PREFIX.BUSINESS,
        AUTH_ERRORS.NUMBER.USER_NOT_FOUND_RESET,
        'auth.user_not_found',
      );
    }
    ...
    const resetPasswordUrl =
      process.env.HOW_URL ||
      `${process.env.HOST}:${process.env.PORT}/auth/reset-password/${token}`;
  ...
  async resetPassword(token: string, password: string) {
    const { userId } = (await this.jwtService.decode(token)) as {
      userId: string;
    };
    ...
    if (!userId) {
      throw new BadRequestError(
        AUTH_ERRORS.PREFIX.BUSINESS,
        AUTH_ERRORS.NUMBER.INVALID_RESET_TOKEN,
  ...
  async verifyEmail(email: string, verificationCode: string) {
    ...
    if (!user) {
      throw new NotFoundError(
        AUTH_ERRORS.PREFIX.BUSINESS,
        AUTH_ERRORS.NUMBER.USER_NOT_FOUND_VERIFY,
        'auth.user_not_found',
      );
    }
    ...
    if (user.isVerified) {
      throw new BadRequestError(
        AUTH_ERRORS.PREFIX.BUSINESS,
        AUTH_ERRORS.NUMBER.USER_ALREADY_VERIFIED,
    ...
    if (user.verificationCode !== verificationCode) {
      throw new BadRequestError(
        AUTH_ERRORS.PREFIX.BUSINESS,
        AUTH_ERRORS.NUMBER.INVALID_VERIFICATION_CODE,
    ...
    if (user.verificationCodeExpiresAt < new Date()) {
      throw new BadRequestError(
        AUTH_ERRORS.PREFIX.BUSINESS,
        AUTH_ERRORS.NUMBER.VERIFICATION_CODE_EXPIRED,
